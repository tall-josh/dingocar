...
def train(cfg, tub_names, model_name, transfer_model, model_type, continuous, aug):
        ...
        while True:
            ...
            for key in keys:
                ...
                if len(batch_data) == batch_size:
                    ...
                    if isTrainSet and (opts["cfg"].TRAIN_AUG is not None):
                        X,y = opts["cfg"].TRAIN_AUG(X,y)
    ...
    train_gen = generator(save_best, opts, gen_records, cfg.BATCH_SIZE, isTrainSet=True)
    val_gen = generator(save_best, opts, gen_records, cfg.BATCH_SIZE, isTrainSet=False)
